<?php 
    ...
    function bulan_view($tgl){
            $bulan = getBulan(substr($tgl,5,2));//ambil bulan pada urutan ke 5 sebanyak 2(5,2) contoh (2018-12-05) hasil yang diambil bulan 12
            return $bulan;       
    }


    //fungsi ini digunakan pada statistik webadmin
    function get_kodebulan($kode){
            $kode_bulan = getBulan(substr($kode,1,2));//sesuaikan kode bulan dari inputan pada halaman statistik_webadmin dan ambil kode bulan urutan ke 1 sebanyak 2(1,2)
            return $kode_bulan;       
    } 
    ...
    function hapus_spasi($s) {
    $c = array (' ');
    $s = str_replace($c, '', $s); // Hilangkan karakter yang telah disebutkan di array $d 
    return $s;
    }
